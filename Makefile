...
dagster:
	tmux send-keys -t dagster.0 "source .venv/bin/activate && source source_env.sh && dbt parse && dagster dev -h 0.0.0.0 -p 3001" ENTER

streamlit:
	tmux send-keys -t streamlit.0 "source .venv/bin/activate && source source_env.sh && cd app/charts && streamlit run main.py" ENTER
