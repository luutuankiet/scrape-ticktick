  ...
  create view "ticktick_gtd"."main"."stg_todos__dbt_tmp" as (
    WITH tasks AS (
    SELECT
        *
    FROM
        "ticktick_gtd"."main"."init_todos"
),
folders AS (
    SELECT
        *
...
SELECT
    md5(cast(coalesce(cast(todo_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) AS todo_key,*
FROM
    renamed
  );
